...
# Rerunning EM and calculating metrics for each run
EM.GP.restartsdata <- function( Y, X, G, m, true_par, z, maxstep=1000, tol=1e-6, restarts=10)
{
  
  best <- list( loglik=-Inf )
  all_restarts <- vector("list", restarts)
  
  summary <- data.frame(
    restart = integer(restarts),
    loglik = numeric(restarts),
    ari = rep(NA_real_, restarts),
    rmse_pi = rep(NA_real_, restarts),
    rmse_sig2 = rep(NA_real_, restarts),
    rmse_l = rep(NA_real_, restarts),
    rmse_tau2 = rep(NA_real_, restarts)
  )
  ...
  for( r in 1:restarts )
  {
    ...
    z_est <- max.col(Z$Gamma.mat)
    summary$restart[r] <- r
    summary$loglik[r] <- Z$loglik
    summary$ari[r] <- mclust::adjustedRandIndex(z, z_est)
    summary$rmse_pi[r] <- Metrics::rmse(true_par$pi.dist, Z$par$pi.dist)
    summary$rmse_sig2[r] <- Metrics::rmse(true_par$sig2, Z$par$psi$sig2)
    summary$rmse_l[r] <- Metrics::rmse(true_par$l, Z$par$psi$l)
    summary$rmse_tau2[r] <- Metrics::rmse(true_par$tau2, Z$par$psi$tau2)
  ...
  return( list(best=best, all_restarts = all_restarts, summary = summary) )
}
